...
- Mobile-first responsive: All UI must be designed for 1 column/375px first, then scale up using sm/md/lg breakpoints.
  - Layout & Spacing Architecture (Shadcn-inspired):
    - Page Master Container: Every page MUST have a single master container wrapping its content (e.g., `<main className="w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 flex flex-col gap-6 md:gap-8">`) to enforce consistent spacing and boundaries. Do not leave elements "flying" outside of containers.
    ...
    - Vertical rhythm: Use `flex flex-col gap-6 md:gap-8` (or `space-y-6 md:space-y-8`) for page-level structural blocks (header to main, section to section) and `gap-4 md:gap-6` for smaller logical blocks. Avoid random top/bottom margins.
